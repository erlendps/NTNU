...
def encoding(node):
    encoding_dict = {}
    encode_from_node(node, "", encoding_dict)
    return encoding_dict

def encode_from_node(node, current_string, encoding_dict):
    if (not has_children(node)):
        encoding_dict[node.character] = current_string
        return
    encode_from_node(node.left_child, current_string + "0", encoding_dict)
    encode_from_node(node.right_child, current_string + "1", encoding_dict)
