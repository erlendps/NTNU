...
static void node_print(node_t *node, int nesting) {
  if (node != NULL) {
    ...
    if (node->symbol)
      printf(" %s(%ld)", ((const char *[]){[SYMBOL_GLOBAL_VAR] = "GLOBAL_VAR", [SYMBOL_GLOBAL_ARRAY] = "GLOBAL_ARRAY", [SYMBOL_FUNCTION] = "FUNCTION", [SYMBOL_PARAMETER] = "PARAMETER", [SYMBOL_LOCAL_VAR] = "LOCAL_VAR"})[node->symbol->type], node->symbol->sequence_number);
