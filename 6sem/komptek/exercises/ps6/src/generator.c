...
/* An intuitive str equals */
int streqs(char *str1, char *str2) {
  return !abs(strcmp(str1, str2));
}

static void generate_relation(node_t *relation) {
  // first compute lhs and save to stack
  generate_expression(relation->children[0]);
  PUSHQ(RAX);
  // now generate rhs
  generate_expression(relation->children[1]);
  // rhs is now stored in %rax, pop from stack to safe register
  POPQ(R10);
  // lastly the cmp instruction
  CMPQ(RAX, R10);
}
...
static void generate_if_statement(node_t *statement) {
  ...
  if_counter++;
  // generate a label indicating start of if statement
  LABEL("STARTIF%lu", local_count);
  // generate the relation
  generate_relation(statement->children[0]);
  char *jmp_label = (statement->n_children == 2) ? "IFTHENEND" : "ELSE";
  // generate the jump instruction, based on the relation operator
  // the jump condition is the negation of the relation.
  char *operator=(char *) statement->children[0]->data;
  if (streqs(operator, "=")) {
    EMIT("jne %s%lu", jmp_label, local_count);
  } else if (streqs(operator, "!=")) {
    EMIT("je %s%lu", jmp_label, local_count);
  } else if (streqs(operator, "<")) {
    EMIT("jge %s%lu", jmp_label, local_count);
  } else if (streqs(operator, ">")) {
    EMIT("jle %s%lu", jmp_label, local_count);
  } else {
  ...
  // ifthen label
  LABEL("IFTHEN%lu", local_count);
  // then generate the "then" statement
  generate_statement(statement->children[1]);
  if (statement->n_children == 2) {
    // IF-THEN statement, just add the label so it can jump here
    LABEL("%s%lu", jmp_label, local_count);
  } else {
    // IF-THEN-ELSE statement, jump to if end label, add the else label,
    // generate the else statement, and then generate the if end label.
    EMIT("jmp IFTHENEND%lu", local_count);
    LABEL("%s%lu", jmp_label, local_count);
    generate_statement(statement->children[2]);
    LABEL("IFTHENEND%lu", local_count);
  }
}
...
static void generate_while_statement(node_t *statement) {
  ...
  generate_relation(statement->children[0]);
  char *operator=(char *) statement->children[0]->data;
  // do a jump based on the relational operator
  if (streqs(operator, "=")) {
    EMIT("jne LOOPEND%lu", local_count);
  } else if (streqs(operator, "!=")) {
    EMIT("je LOOPEND%lu", local_count);
  } else if (streqs(operator, "<")) {
    EMIT("jge LOOPEND%lu", local_count);
  } else if (streqs(operator, ">")) {
    EMIT("jle LOOPEND%lu", local_count);
  } else {
    // somethings not right
    destroy_tables();
    destroy_syntax_tree();
  ...
  // generate loop body
  generate_statement(statement->children[1]);
  // jmp back to the check and add an loopend label
  EMIT("jmp LOOPCHECK%lu", local_count);
  LABEL("LOOPEND%lu", local_count);
  // decrement break_loop_id since we have now "exited" the loop
  break_loop_id--;
}

static void generate_break_statement() {
  // break loop id contains the outermost loop
  // break_loop_id == -1 implies we are not in a loop
  assert(break_loop_id >= 0);
  EMIT("jmp LOOPEND%li", break_loop_id);
}
